...
export class Decimal extends ElementBase<string> {
    ...
    constructor(options: ElementOptions<string>) {
        ...
        this.type = ElementType.Decimal;
        this.validation = options.validation;
    }

    addValidators(formValidationService: FormValidationService, validators: Array<ValidatorFn>): void {        
        formValidationService.addMaximumValueValidator(this.validation, validators);
        formValidationService.addMinimumValueValidator(this.validation, validators);
        formValidationService.addRequiredValidator(this.isRequired, validators);
    }

    static createInstance(key: string, numberService: NumberService, template: {}): Decimal {
        let decimal = new Decimal({
            ...
            validation: {
                maximumValue: template[elementDesign.maxValue],
                minimumValue: template[elementDesign.minValue]
            }
        });
